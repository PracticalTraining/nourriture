...
public class AdminRestfulService {
...

	{
		...
		RestfulServiceUtil.addChildrenLinks(adminChildrenLinks, "login",
				"/login", "GET");
		RestfulServiceUtil.addChildrenLinks(adminChildrenLinks,
	...
	public String login(@QueryParam("name") String name,
		...
		final int ERROR_CODE_USER_NOT_EXIST = -2;
		final int ERROR_CODE_PASSWORD_NOT_VALIDATED = -3;
	...
	/*
	 * public String getById(@PathParam("id") int id){ JsonObject ret = new
	 * JsonObject(); //select from database Admin admin= adminDao.getById(id);
	 * if(admin != null) { JsonObject jAdmin = transformAdmin(admin);
	 * ret.add("admin", jAdmin); } ret.add("links", idChildrenLinks); return
	 * ret.toString(); }
	 */
	/*
	 * private JsonObject transformAdmin(Admin admin) { // TODO Auto-generated
