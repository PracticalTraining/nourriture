...
public class CustomerRestfulService {
...

	{
		...
		RestfulServiceUtil.addChildrenLinks(customerChildrenLinks,
				"search customer", "/search", "GET");
		RestfulServiceUtil.addChildrenLinks(customerChildrenLinks, "login",
				"/login", "GET");
		RestfulServiceUtil.addChildrenLinks(customerChildrenLinks,
				"get customer according to id", "/{id}", "GET");
		RestfulServiceUtil.addChildrenLinks(customerChildrenLinks,
				"update customer according to id", "/{id}", "PUT");
		searchChildrenLinks = new JsonArray();
		loginChildrenLinks = new JsonArray();
		idChildrenLinks = new JsonArray();
		RestfulServiceUtil.addChildrenLinks(idChildrenLinks,
				"get customer interests", "/{interest}", "GET");
		RestfulServiceUtil.addChildrenLinks(idChildrenLinks,
				"add a customer interest", "/{interest}", "POST");
		RestfulServiceUtil.addChildrenLinks(idChildrenLinks,
				"delete a customer interest", "/{interest}", "DELETE");
		interestChildrenLinks = new JsonArray();
	}

	/** add a customer **/
	@POST
	public String addCustomer(@FormParam("name") String name,
			@FormParam("password") String password, @FormParam("sex") int sex,
		...
		if (name == null || name.equals("") || password == null
				|| password.equals("") || (sex != 0 && sex != 1) || age < 0) {
	...
	@GET
	@Path("{id}/{interest}")
	public String getInterests(@PathParam("id") int id,
		...
		if (id <= 0
				|| interest == null
				|| interest.equals("")
				|| (!interest.equals("flavour")
						&& !interest.equals("foodCategory") && !interest
							.equals("recipeCategory"))) {
			ret.addProperty("errorCode", ERROR_CODE_BAD_PARAM);
		...
		if (interest.equals("flavour")) {
			...
			if (sIds == null || sIds.equals("")) {
				...
				for (String sid : ids) {
					...
					JsonObject flavour = JsonUtil.beanToJson(flavourDao
							.getById(fId));
			...
			if (sIds == null || sIds.equals("")) {
				...
				for (String sid : ids) {
					int fId = Integer.parseInt(sid);
					JsonObject foodCategory = JsonUtil
							.beanToJson(foodCategoryDao.getById(fId));
			...
			if (sIds == null || sIds.equals("")) {
				...
				for (String sid : ids) {
					int fId = Integer.parseInt(sid);
					JsonObject recipeCategory = JsonUtil
							.beanToJson(recipeCategoryDao.getById(fId));
	...
	@POST
	@Path("{id}/{interest}")
	public String addInterest(@PathParam("id") int id,
			@PathParam("interest") String interest, @FormParam("id") int InId) {
		JsonObject ret = new JsonObject();
		// define error code
		final int ERROR_CODE_USER_NOT_EXIST = -1;
		...
		if ((interest.equals("flavour") && flavourDao.getById(InId) == null)
				|| (interest.equals("foodCategory") && foodCategoryDao
						.getById(InId) == null)
				|| (interest.equals("recipeCategory") && recipeCategoryDao
						.getById(InId) == null)) {
			...
			if (sIds != null) {
				if (sIds.contains(String.valueOf(InId))) {
					ret.addProperty("errorCode",
							ERROR_CODE_INTEREST_ALREADY_SET);
					ret.add("links", interestChildrenLinks);
					return ret.toString();
				}
			}
	...
	public String deleteInterest(@PathParam("id") int id,
		...
		if (interest.equals("flavour")) {
			String sIds = customer.getInterestFlavourIds();
			if (sIds == null || sIds.equals("")
					|| !sIds.contains(String.valueOf(InId))) {
				ret.addProperty("errorCode", ERROR_CODE_INTEREST_NOT_SET);
				ret.add("links", interestChildrenLinks);
				return ret.toString();
			}
		...
		if (interest.equals("flavour")) {
			String sIds = customer.getInterestFlavourIds();
			customer.setInterestFlavourIds(RestfulServiceUtil
					.deleteIdFromIdList(InId, sIds));
		} else if (interest.equals("foodCategory")) {
			String sIds = customer.getInterestFoodCategoryIds();
			customer.setInterestFoodCategoryIds(RestfulServiceUtil
					.deleteIdFromIdList(InId, sIds));
		} else {
			String sIds = customer.getInterestRecipeCategoryIds();
			customer.setInterestRecipeCategoryIds(RestfulServiceUtil
					.deleteIdFromIdList(InId, sIds));
		}
		customerDao.update(customer);
		ret.addProperty("result", 0);
	...
	/**
	 * transform customer from bean to json,customer interest ids will be
	 * transformed to detail information
	 * 
	 * @param bean
	 *            form of customer
	...
	private JsonObject transformCustomer(Customer customer) {
		...
		if (str != null && !str.equals("")) {
			...
			for (String id : ids) {
				int fId = Integer.parseInt(id);
				JsonObject foodCategory = JsonUtil.beanToJson(foodCategoryDao
						.getById(fId));
				foodCategory.remove("topCategory");
				foodCategory.remove("superiorCategoryId");
