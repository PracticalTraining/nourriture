...
public class RecipeRestfulService {
	...
	{
		RestfulServiceUtil.addChildrenLinks(recipeChildrenLinks,
				"get recipe according to id", "/{id}", "GET");
		RestfulServiceUtil.addChildrenLinks(recipeChildrenLinks,
				"update recipe according to id", "/{id}", "PUT");
		RestfulServiceUtil.addChildrenLinks(recipeChildrenLinks,
				"delete recipe according to id", "/{id}", "DELETE");
	}
	...
	public String updateRecipe(@PathParam("id") int id,
			...
			@FormParam("ingredient") String ingredient,
			@FormParam("picture") String picture,
			@FormParam("customerId") int customerId,
			@FormParam("catogeryId") int catogeryId) {
		...
		Customer my_customer = this.customerDao.getById(my_recipe
				.getCustomerId());
		JsonObject ret = new JsonObject();

		if (id <= 0 || name.equals("") || name == null
				|| description.equals("") || description == null
				|| ingredient.equals("") || ingredient == null
				|| picture.equals("") || picture == null || customerId <= 0
				|| catogeryId <= 0) // check request parameters
		{
	...
	@POST
	public String addRecipe(@FormParam("name") String name,
			@FormParam("description") String description,
			@FormParam("ingredient") String ingredient,
			@FormParam("picture") String picture,
			@FormParam("customerId") int customerId,
		...
		RecipeCategory my_recipeCategory = this.recipeCategoryDao
				.getById(catogeryId);

		if (name.equals("") || name == null || description.equals("")
				|| description == null || ingredient.equals("")
				|| ingredient == null || picture.equals("") || picture == null
				|| customerId <= 0 || catogeryId <= 0) // check request
														// parameters
		{
			ret.addProperty("errorCode", ERROR_CODE_BAD_PARAM);
			ret.add("links", this.recipeChildrenLinks);
		...
		if (my_recipeCategory == null) { // check if Recipe exsits
			...
			ret.addProperty("errorCode",
					ERROR_CODE_RECIPE_CATEGORY_DOES_NOT_EXISTS);
			ret.add("links", this.recipeChildrenLinks);
			return ret.toString();
		}
	...
	@GET
	@Path("getPage")
	public String getPage(@QueryParam("categoryId") int categoryId,
	...
	public String recommendByInterest(@QueryParam("customerId") int customerId,
		...
		String interestRecipeCategoryIds = customer
				.getInterestRecipeCategoryIds();
		int[] categoryIds = null;
		if (interestRecipeCategoryIds != null
				&& !interestRecipeCategoryIds.equals("")) {
