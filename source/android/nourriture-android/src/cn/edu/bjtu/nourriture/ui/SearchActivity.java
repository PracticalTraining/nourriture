...
public class SearchActivity extends BaseActivity {
	...
	protected void onStart() {
		...
		try {
			SqlInfo sqlInfo = new SqlInfo("select * from search_history order by time desc");
	...
	private final void search(String keyword){
		...
		try {
			...
			if(list.size() == 0)
				...
				sqlInfo = new SqlInfo("update search_history set time = ? where id = ?");
	...
	private class SearchThread extends Thread{
		...
		public void run() {
			...
			try {
				String foodsStr = httpUtils.sendSync(HttpMethod.GET, foodUrl, params).readString();
				String recipesStr = httpUtils.sendSync(HttpMethod.GET, recipeUrl, params).readString();
				Message message = mSearchHandler.obtainMessage();
				Bundle bundle = new Bundle();
				bundle.putString("foods", new JSONObject(foodsStr).getJSONArray("foods").toString());
